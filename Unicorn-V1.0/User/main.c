...
static void vTaskFLASH(void *pvParameters)
{
    while(1)
    {
        ...
        if(flag_LED==true)
        {
           ...
           Led1_Set(LED_ON);vTaskDelay(500);Led1_Set(LED_OFF);		//µãÁÁLED4£¬²¢ÑÓÊ±500ms£¬È»ºóÏ¨ÃğLED4
               
           Led2_Set(LED_ON);vTaskDelay(500);Led2_Set(LED_OFF);		//µãÁÁLED5£¬²¢ÑÓÊ±500ms£¬È»ºóÏ¨ÃğLED5
                      
           Led3_Set(LED_ON);vTaskDelay(500);Led3_Set(LED_OFF);		//µãÁÁLED6£¬²¢ÑÓÊ±500ms£¬È»ºóÏ¨ÃğLED6
                      
           Led4_Set(LED_ON);vTaskDelay(500);Led4_Set(LED_OFF);		//µãÁÁLED7£¬²¢ÑÓÊ±500ms£¬È»ºóÏ¨ÃğLED7
           flag_LED=false;
        }
        if(flag_Beep==true)
        {
             ...
             Beep_Set(BEEP_ON);
             vTaskDelay(500);
             Beep_Set(BEEP_OFF);           //´ò¿ª·äÃùÆ÷,¹Ø±Õ·äÃùÆ÷
             flag_Beep=false;
       }
